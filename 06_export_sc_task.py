...
def load_downloaded() -> set:
    """Load the set of already downloaded SCTASK numbers from log file"""
...
def mark_downloaded(sctask_number: str):
    """Mark a SCTASK number as downloaded by appending to log file"""
    with open(DOWNLOADED_LOG, 'a', encoding='utf-8') as f:
        f.write(f"{sctask_number}\n")
        f.flush()  # Ensure it's written immediately

def main():
    OUT.mkdir(parents=True, exist_ok=True)

    # Load already downloaded SCTASK numbers for resume capability
    downloaded = load_downloaded()
    if downloaded:
        print(f"Found {len(downloaded)} already downloaded SCTASK(s). Will skip them.")
    else:
    ...
    with sync_playwright() as p:
        ...
        # เข้า SC Task list
        page.goto(SCTASK_LIST_URL, wait_until="domcontentloaded")
        ...
        try:
            ...
            page.screenshot(path="debug_sctask_list_page.png")
            print("\nPossible causes:")
            print("1. Session expired - run: python 01_login_save_state.py")
            print("2. Wrong URL or page structure changed")
            print("3. Page is still loading - check debug_sctask_list_page.png")
            browser.close()
            raise
            ...
            if control_button is not None:
                ...
                changed = frame.evaluate("""
                    () => {
                        ...
                        let controlBtn = document.querySelector('button#sc_task_control_button');
                        if (!controlBtn) {
        ...
        while True:
            ...
            for i in range(count):
                ...
                # คลิกที่ SCTASK Number ลิงก์ตัวแรกในแถว
                ...
                try:
                    ...
                    print(f"[WARN] Could not read SCTASK number for row {i+1}: {e}")
                    continue

                if not number:
                    print(f"[WARN] Empty SCTASK number in row {i+1}, skipping")
                    continue
                ...
                # Mark this SCTASK as downloaded (for resume capability)
